...
st.set_page_config(page_title="Hệ thống Nhật ký V18.0", layout="wide")
st.title("🚛 HỆ THỐNG TÍNH TOÁN & CÂN ĐỐI NHẬT KÝ (V18.0)")

# =====================================================================
# I. CÁC HÀM TIỆN ÍCH HỆ THỐNG
# =====================================================================
...
# =====================================================================
# II. LÕI THUẬT TOÁN CÂN ĐỐI (CORE V18)
# =====================================================================

def balance_logic(df_input):
    df = df_input.copy()
    
    c_km = get_c(df, ['km di chuyển'])
    c_h = get_c(df, ['giờ tại chỗ'])
    c_ton_dau = get_c(df, ['tồn đầu'])
    c_cap = get_c(df, ['số cấp'])
    c_tt = get_c(df, ['tiêu thụ'])
    c_thieu = get_c(df, ['thiếu hụt'])
    c_ton_cuoi = get_c(df, ['tồn cuối'])
    
    required = [c_km, c_h, c_ton_dau, c_cap, c_tt, c_thieu, c_ton_cuoi]
    if not all(required):
        return None, "❌ File không đúng cấu trúc Nhật ký thi công (Thiếu cột tính toán)."

    for idx, row in df.iterrows():
        try:
            val_thieu = float(row[c_thieu]) if pd.notnull(row[c_thieu]) else 0.0
            if val_thieu > 0.0001:
                thuc_co = float(row[c_ton_dau]) + float(row[c_cap])
                nhu_cau = float(row[c_tt])
                
                ratio = thuc_co / nhu_cau if nhu_cau > 0 else 0
                
                df.at[idx, c_km] = float(row[c_km]) * ratio
                df.at[idx, c_h] = float(row[c_h]) * ratio
                df.at[idx, c_tt] = thuc_co
                df.at[idx, c_thieu] = 0.0
                df.at[idx, c_ton_cuoi] = 0.0
        except Exception:
            continue
            
    return df, "✅ Đã cân đối thành công: Triệt tiêu mọi dòng thiếu hụt nhiên liệu!"

# =====================================================================
# III. GIAO DIỆN VÀ ĐIỀU HƯỚNG
# =====================================================================

tab1, tab2 = st.tabs(["🚀 GIAI ĐOẠN 1: TÍNH TOÁN (V17)", "⚖️ GIAI ĐOẠN 2: CÂN ĐỐI (V18)"])

# ---------------------------------------------------------------------
# TAB 1: LÕI TÍNH TOÁN V17 (TRỌN VẸN)
# ---------------------------------------------------------------------
with tab1:
    with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            if errors:
                ...
                st.success("✅ LỚP KHIÊN KIỂM DUYỆT THÔNG QUA: Dữ liệu sạch 100%. Sẵn sàng phân bổ.")
                # =====================================================================
                # BẮT ĐẦU TÍNH TOÁN V17
                # =====================================================================
                if st.button("🚀 TẠO NHẬT KÝ & PHIẾU CẤP (V17.0)", type="primary", use_container_width=True):
                    ...
                    # --- HIỂN THỊ KẾT QUẢ VÀ LƯU SESSION ---
                    st.divider()
                    ...
                    if res_nk:
                        st.success("✅ Đã tính toán xong Nhật ký V17. Bạn có thể sang Tab 2 để thực hiện Cân đối số liệu.")
                        sub_t1, sub_t2 = st.tabs(["📊 BẢNG 1: NHẬT KÝ THI CÔNG", "📑 BẢNG 2: PHIẾU CẤP"])
                        df_nk = pd.DataFrame(res_nk)
                        df_pc = pd.DataFrame(res_phieu_cap)
                        
                        # Lưu vào Session State cho Tab 2 dùng
                        st.session_state['df_v17'] = df_nk
                        st.session_state['df_pc_v17'] = df_pc
                        
                        with sub_t1: st.dataframe(format_vietnamese_style(df_nk), height=600, use_container_width=True)
                        with sub_t2: st.dataframe(format_vietnamese_style(df_pc), height=600, use_container_width=True)
                        ...
                        st.download_button("📥 Tải Xuống File Excel (V17.0 Chưa Cân Đối)", output.getvalue(), "Ket_Qua_V17.xlsx")
                    else:
...
# ---------------------------------------------------------------------
# TAB 2: GIAO DIỆN CÂN ĐỐI (V18)
# ---------------------------------------------------------------------
with tab2:
    st.header("⚖️ BỘ CÔNG CỤ CÂN ĐỐI & LÀM ĐẸP HỒ SƠ")
    st.markdown("Chức năng này giúp tự động bóp giảm Km/Giờ thi công tỷ lệ thuận với lượng nhiên liệu đang có thực tế, nhằm đưa toàn bộ các xe đang bị nợ/thiếu hụt về **Tồn cuối = 0**.")
    
    st.subheader("1. Nguồn Dữ Liệu Đầu Vào")
    source = st.radio("Chọn nguồn dữ liệu cần cân đối:", 
                     ["🔄 Sử dụng luôn kết quả Nhật ký vừa tính ở Tab 1", 
                      "📂 Tải lên File Excel kết quả đã tính toán từ trước"])
    
    df_to_balance = None
    
    if "🔄" in source:
        if 'df_v17' in st.session_state:
            df_to_balance = st.session_state['df_v17']
            st.info("✅ Đã nhận diện được dữ liệu Nhật ký từ Tab 1.")
        else:
            st.warning("⚠️ Chưa có dữ liệu. Vui lòng chạy tính toán ở Tab 1 trước.")
            
    else:
        uploaded_v18 = st.file_uploader("Nạp File Excel (Sheet NhatKy)", type=["xlsx"])
        if uploaded_v18:
            try:
                df_to_balance = pd.read_excel(uploaded_v18)
                st.info("✅ Đã tải dữ liệu từ File Excel.")
            except Exception as e:
                st.error(f"Lỗi đọc file: {e}")

    # Nút thực hiện cân đối
    st.divider()
    if df_to_balance is not None:
        if st.button("🚀 KÍCH HOẠT THUẬT TOÁN CÂN ĐỐI", type="primary", use_container_width=True):
            with st.spinner("Đang tính toán tỷ lệ và làm đẹp số liệu..."):
                df_balanced, msg = balance_logic(df_to_balance)
                
            if df_balanced is not None:
                st.success(msg)
                
                # Hiển thị
                st.markdown("### 📊 BẢNG NHẬT KÝ (ĐÃ CÂN ĐỐI TỒN CUỐI VỀ 0)")
                st.dataframe(format_vietnamese_style(df_balanced), height=600, use_container_width=True)
                
                # Xuất file kết hợp
                out_v18 = io.BytesIO()
                with pd.ExcelWriter(out_v18, engine='openpyxl') as writer:
                    df_balanced.to_excel(writer, index=False, sheet_name='NhatKy_CanDoi')
                    # Nếu có phiếu cấp lưu ở session thì đính kèm luôn cho tiện
                    if 'df_pc_v17' in st.session_state:
                        st.session_state['df_pc_v17'].to_excel(writer, index=False, sheet_name='PhieuCap_GiuNguyen')
                
                st.download_button("📥 TẢI XUỐNG FILE HỒ SƠ ĐÃ LÀM ĐẸP (V18.0)", out_v18.getvalue(), "Ho_So_Hoan_Thien_V18.xlsx")
            else:
                st.error(msg)
